...
if os.path.exists(config_path):
    ...
    print(
        f"{TerminalColors.YELLOW}Warning: config.json not found. Using default config for development/testing.{TerminalColors.RESET}"
    )
...
def fetch_market_data(exchange, symbol, timeframe="1h", limit=100):
    ...
    try:
        ...
        if exchange.id == "bitfinex" and exchange.options.get("paper", False):
            ...
            log.info(
                f"Använder paper trading symbol: {formatted_symbol} (ursprungligen {symbol})"
            )
...
def place_order(
    ...
    log.order(
        f"Försöker lägga {order_type}-order: symbol: {symbol}, amount: {amount}, price: {price}"
    )
    ...
    try:
        ...
        if EMAIL_NOTIFICATIONS:
            ...
            log.notification(
                f"E-postnotifiering skickad för order {order.get('id', 'N/A') if order else 'N/A'}"
            )
            ...
            while True:
                ...
                if isinstance(data, list) and len(data) > 1 and data[1] == "oc":
                    ...
                    if EMAIL_NOTIFICATIONS and (
                        ...
                        send_email_notification(subject, body)
                        log.notification(
                            f"E-postnotifiering skickad för order {order_id}"
                        )
                ...
                # Hantera autentiseringssvar
                    ...
                    if data.get("status") == "OK":
                        log.websocket(
                            f"Autentisering lyckades! Användare: {data.get('userId', 'Okänd')}"
                        )
                    else:
...
if __name__ == "__main__":
    ...
    for attempt in range(HEALTH_PORT, HEALTH_PORT + 5):
        ...
        log.error(
            "Misslyckades att starta health-check server på alla försökta portar."
        )
        sys.exit(1)
...
def run_backtest(
    ...
    # Generated by Copilot
    # ...existing code...
    # Use global exchange instance if not provided
    global exchange
...
class TradingBot:
    ...
    def place_order(self, symbol=None, order_type=None, amount=None, price=None):
        ...
        logger.info(
            f"Placerar order: {order_type} {amount} {symbol} @ {price or 'marknadspris'}"
        )
    ...
    def get_orders(self, symbol=None, start_date=None, end_date=None):
        ...
        """
        logger.info(
            f"Hämtar ordrar med filter - symbol: {symbol}, start_date: {start_date}, end_date: {end_date}"
        )
    ...
    def get_logs(self, limit=10):
        ...
        try:
            ...
            logs.append(
                {"timestamp": datetime.now().isoformat(), "message": "Bot startad"}
            )
            logs.append(
                {
                    "timestamp": datetime.now().isoformat(),
                    "message": "Anslutning till API upprättad",
                }
            )
            return logs
        except Exception as e:
    ...
    def get_price_history(self, symbol=None, timeframe="1h", limit=100):
        ...
        logger.info(
            f"Hämtar prishistorik för {symbol}, timeframe {timeframe}, limit {limit}"
        )
    ...
    def analyze_strategy_performance(
        ...
        """
        logger.info(
            f"Analyserar strategi-prestanda med filter - symbol: {symbol}, start_date: {start_date}, end_date: {end_date}"
        )
        ...
        try:
            ...
            if decision == "hold":
                ...
                return {
                    "action": "hold",
                    "price": current_price,
                    "reason": "Prisnivå indikerar att vi bör hålla positionen",
                }
            ...
            order_result = self.place_order(
                symbol=symbol, order_type=decision, amount=amount, price=current_price
            )
    ...
    def _execute_ma_strategy(self):
        ...
        try:
            ...
            history = self.get_price_history(
                symbol, timeframe="1h", limit=long_period + 10
            )
            ...
            if len(prices) < long_period:
                return {
                    "error": f"Otillräcklig prishistorik för MA-strategi. Behöver minst {long_period} punkter."
                }
...
def create_order(symbol, order_type, side, amount, price=None):
    ...
    try:
        ...
        with open("order_status_log.txt", "a") as f:
            f.write(
                f"{datetime.now()}: Created order - Symbol: {symbol}, Type: {order_type}, Side: {side}, Amount: {amount}, Price: {price}, Order ID: {order['id'] if 'id' in order else 'N/A'}\n"
            )
...
def cancel_order(order_id, symbol):
    ...
    try:
        ...
        with open("order_status_log.txt", "a") as f:
            f.write(
                f"{datetime.now()}: Canceled order - Order ID: {order_id}, Symbol: {symbol}\n"
            )
